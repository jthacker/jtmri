# Source: http://scipy-central.org/item/22/4/building-a-simple-interactive-2d-data-viewer-with-matplotlib
    ...
    def click(self,event):
        ...
        if event.inaxes==self.overview:
            ...
            if event.button==1:
                ...
                self.overview.axvline(xpos+0.5,color=c.get_color(),lw=2)

            elif event.button==3:
                #Plot it                
                c,=self.x_subplot.plot(self.x, self.z[ypos,:],label=str(self.y[ypos]))
                self.overview.axhline(ypos+0.5,color=c.get_color(),lw=2)
