...
class MorphACWE(object):
    ...
    def nstep(self, iterations):
        """Run several iterations of the morphological Chan-Vese method."""
        for i in xrange(iterations):
            self.step()

    def _check_convergence(self, prev_mask, curr_mask, thresh):
        """Check if the mask has converged
        Args:
            prev_mask -- previous mask
            curr_mask -- current maks
            thresh    -- threshold fraction of number of pixels that can change
                         (0 <= thresh <= 1)
        Returns:
            True if the difference in pixels between consecutive masks
        has changed by less than the threshold amount
        """
        assert 0 <= thresh <= 1, 'Threshold should be in [0, 1]'
        frac = np.abs(prev_mask - curr_mask).mean()
        log.debug('checking convergence: frac: {} thresh: {}'\
            .format(frac, thresh))
        return frac < thresh

    def run(self, max_iterations=None, thresh=None):
        """Automatically run the steps, stopping when the stopping criterion is satisfied"""
        prev_mask = self._u
        curr_mask = self._u
        converged_count = 0
        ...
        for i in xrange(1, max_iterations + 1):
            curr_mask = self.step()
            if self._check_convergence(prev_mask, curr_mask, thresh):
                converged_count += 1
            else:
                converged_count = 0

            if converged_count >= 3:
                log.debug('ACWE has converged')
                break
            prev_mask = curr_mask
        log.debug('ACWE finished. iterations: {}, max_iterations: {}, thresh: {}'\
                .format(i, max_iterations, thresh))
        return curr_mask
